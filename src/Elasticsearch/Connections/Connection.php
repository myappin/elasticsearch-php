...
class Connection implements ConnectionInterface {
    ...
    private function process4xxError($request, $response, $ignore) {
        ...
        else if ($statusCode === 400 && strpos($responseBody, 'script_lang not supported') !== false) {
    ...
    private function process5xxError($request, $response, $ignore) {
        ...
        else if ($statusCode === 500 && preg_match('/ActionRequestValidationException.+ no documents to get/', $responseBody) === 1) {
            $exception = new NoDocumentsToGetException($exception->getMessage(), $statusCode, $exception);
        }
        else if ($statusCode === 500 && strpos($responseBody, 'NoShardAvailableActionException') !== false) {
    ...
    private function tryDeserializeError($response, $errorClass) {
        ...
        $error = $this->serializer->deserialize($responseBody, $response['transfer_stats']);
        if (is_array($error) === true) {
            ...
            if (isset($error['error']['reason']) === true) {
                ...
                $original = new $errorClass($responseBody, $response['status']);
            ...
            return new $errorClass($responseBody, $response['status']);
        }
